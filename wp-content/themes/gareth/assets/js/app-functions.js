...
function resizeWindowCallback(){
    ...
    if(!appVars.tablesSwapped && appVars.mobile){
...
/* Functions for showing and hiding the tooltips */
function showTip(obj){


    if(appVars.mobile) { return; }
    if(typeof( $(obj) ) === 'undefined' || $(obj).attr('title') == ''){ return; }
    if( $(obj).hasClass('no-tip') ){ return; } 

    //position and fill the popup
    var tooltip = $('.tooltip'),
        newLeft = 0,
        newTop = 0;
    tooltip.html($(obj).attr('title'));
    tooltip.show();

    // top is top - half height of area calling it + half of height of toltip
    newTop = ($(obj).offset().top - ( $(obj).outerHeight() / 2)) + (tooltip.outerHeight() / 2) - 2;
    newLeft = ( $(obj).offset().left + $(obj).outerWidth() ) + 10;

    // position that bad boi
    tooltip.css({
        'top':newTop,
        'left':newLeft,
        'display':'block'
...
// Function to hide all the tooltips
function hideTip(){
    $('.tooltip').hide();
}
